...
function Home() {
    ...
    const fetchMore = () => {
      constr.search(query, types, limit, offset, {}, (error, data) => {
        if (error) {
          ...
          // Filter out user's playlists and those containing the ignore keyword from the search results
          const filteredPlaylists = data.playlists.items.filter(playlist =>
            !userPlaylists.includes(playlist.id) && !playlist.name.toLowerCase().includes(ignoreKeyword.toLowerCase())
          );
          totalResults = totalResults.concat(filteredPlaylists);
          console.log("filteredPlaylists", filteredPlaylists)
          if (totalResults.length < 100 && data.playlists.items.length === limit) {
  ...
  const handleIgnoreKeywordChange = (event) => {
    setIgnoreKeyword(event.target.value);
  };
        ...
        {!accessToken ? (
          ...
          <>
            ...
            <div className="flex flex-col items-center justify-center font-gunnar" style={{ width: '100%', marginTop: '50px', position: 'relative' }}>
                ...
                <div className="relative w-full max-w-[500px] h-[75px] mx-auto mt-4">
                    <div className="absolute bg-transparent border-5 border-custom-dark-purple rounded-30 flex items-center justify-center w-full h-[75px]">
                        <div className="w-full h-full bg-white rounded-25 box-border" />
                    </div>
                    ...
                    <div className="flex w-full h-full">
                        ...
                        <input
                            ...
                            value={ignoreKeyword}
                            onChange={handleIgnoreKeywordChange}
                            style={{
                            padding: '0 10px',
                            outline: 'none',
                            boxShadow: 'none',
                ...
                {duplicateTracks.length > 0 && (
                    <div
                        className="mt-4 w-2/3 md:w-full max-w-[500px] flex justify-center absolute top-[200px] overflow-y-scroll bg-white shadow-lg rounded-2xl"
                        style={{ height: '200px' }}
                    >
                        <ul className="list-inside text-custom-dark-purple text-left p-6 w-full" style={{ height: '200px' }}>
                        {duplicateTracks.map((track) => (
                ...
                {duplicateTracks.length > 0 && (
                <div className="flex justify-center mt-4" style={{ position: 'absolute', top: '420px' }}>
                  <button
                    className="btn btn-primary transition-shadow duration-300 ease-in-out hover:shadow-lg"
                    onClick={createPlaylist}
                    style={{
